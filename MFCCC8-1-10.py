...
def skill_factor(ranked1, ranked2, population):
    ...
    for ind in population:
        # Find the index of the individual in both ranked lists
        rank1 = ranked1.index(population.index(ind))
        rank2 = ranked2.index(population.index(ind))

        # Choose the best task for each individual
        skill_factor = rank1 if rank1 < rank2 else rank2
...
def scalar_fitness(ranked1, ranked2, skill_factors):
    ...
    """
    return [1 / (ranked1[skill_factors[i]] + 1) for i in range(len(skill_factors))]
...
def optimize_mfo_ccc(pop_size, lower_bound_1, upper_bound_1, lower_bound_2, upper_bound_2, generations=100):
    # Initialize population
    population = initialize_population(pop_size, lower_bound_1, upper_bound_1) + initialize_population(pop_size,
                                                                                                       lower_bound_2,
                                                                                                       upper_bound_2)
    ...
    for generation in range(generations):
        ...
        for i in range(1, len(population)):
            # Cooperation updates based on sharing information
...
# Run optimization
pop_size = 50
lower_bound_1, upper_bound_1 = 0, 9
lower_bound_2, upper_bound_2 = 0, 900

optimal_x = optimize_mfo_ccc(pop_size, lower_bound_1, upper_bound_1, lower_bound_2, upper_bound_2)
